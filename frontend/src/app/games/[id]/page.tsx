...
export default function PlayGame() {
    ...
    const playerName = id ? getPlayerNameFromLocalStorage(id) : "";
    ...
    if (!id) return <div className="text-black">invalid game id</div>;
    if (isLoading) return <div className="text-black">loading...</div>;
    if (!data || !playerName) return <div className="text-black">no data</div>;
    if (!data.player2) return <WaitingPlayerToJoin id={id} />;
...
function GameInfo({ gameData, setGameData, playerName, }: { gameData: GameData, setGameData: Dispatch<SetStateAction<GameData | null>>; playerName: string; }) {
    ...
    const handleReplayGame = () => {
        if (replayInProgress || !gameData.moves.length) {
            return;
        }
        ...
        const N = 6;
        const M = 7;
        const emptyBoard = Array.from({ length: N }, () => Array(M).fill(0));
        const finalBoard = gameData.board;
        const replayDelayMs = 500;

        setGameData((prevState: GameData | null) => {
            if (!prevState) return prevState;
            return { ...prevState, board: emptyBoard, move_number: 0 };
        });
        ...
        gameData.moves.forEach((move: MoveData, i: number) => {
            setTimeout(() => {
                setGameData((prevState: GameData | null) => {
                    ...
                    const nextBoard = prevState.board.map((row) => [...row]);
                    nextBoard[move.row][move.col] = move.val;

                    return { ...prevState, board: nextBoard, move_number: i + 1 };
                });
            }, (i + 1) * replayDelayMs);
        });
        ...
        setTimeout(() => {
            ...
            setGameData((prevState: GameData | null) => {
                if (!prevState) return prevState;
                return { ...prevState, board: finalBoard, move_number: gameData.moves.length };
            });
        }, (gameData.moves.length + 1) * replayDelayMs);
    };
